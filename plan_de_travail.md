...
- [x] **Changement de Livreur & Réouverture de la Demande (Design Épuré Icone Unique `+`)** : Le client / boutique dispose d'un bouton **`🔄 Changer de Livreur`** et d'un bouton d'action épuré **`Publier une Autre Livraison`** avec un seul icône `+` ([client/page.tsx](file:///c:/Users/HP/Documents/Livraison/src/app/client/page.tsx), [change-driver/route.ts](file:///c:/Users/HP/Documents/Livraison/src/app/api/deliveries/%5Bid%5D/change-driver/route.ts)).
