...
function ClientMessagesContent() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-slate-50 to-blue-50/30 text-slate-900 pb-12">
      {/* NAVBAR HEADER */}
      <header className="border-b border-slate-200/80 bg-white/90 backdrop-blur-xl sticky top-0 z-40 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-3.5 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Link href="/client" className="p-2 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">Vos échanges en direct avec les livreurs</p>
            </div>
          </div>
          ...
          <div className={`border-r border-slate-200/80 p-4 space-y-4 bg-slate-50/50 ${selectedConv ? 'hidden md:flex flex-col' : 'flex flex-col'}`}>
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-2xl text-xs outline-none focus:ring-2 focus:ring-blue-500 shadow-sm placeholder-slate-400"
                />
              </div>
            </div>
            ...
            {conversations.length > 0 && (
              <div className="space-y-1.5 pt-1">
                <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider px-1">Contacts récents</div>
                <div className="pb-2 border-b border-slate-200/60 overflow-x-auto scrollbar-none flex items-center space-x-3 pr-2">
                  {conversations.slice(0, 6).map((c) => {
                    ...
                    const initial = p.fullName ? p.fullName[0].toUpperCase() : 'L';
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 scrollbar-thin">
              {loading ? (
                ...
                filteredConvs.map((c) => {
                  ...
                  const initial = partner.fullName ? partner.fullName[0].toUpperCase() : 'L';
                    ...
                    <button
                      ...
                      className={`w-full p-3.5 rounded-2xl text-left transition-all flex items-center gap-3.5 border cursor-pointer ${
                        isSelected
                          ...
                          : 'bg-white border-slate-200/80 text-slate-800 hover:border-blue-300 hover:shadow-sm'
                      }`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-0.5">
                          <span className={`font-bold text-xs truncate ${isSelected ? 'text-white' : 'text-slate-900'}`}>
                            {partner.fullName || 'Livreur'}
                          </span>
                          <span className={`text-[10px] font-mono ${isSelected ? 'text-blue-100' : 'text-slate-400'}`}>
                            12:30
                          </span>
                        </div>
                        <p className={`text-xs truncate ${isSelected ? 'text-blue-100 font-medium' : 'text-slate-500'}`}>
                          {lastMsg}
                        </p>
                      </div>
                    </button>
          ...
          <div className={`md:col-span-2 p-4 md:p-6 flex flex-col justify-between bg-slate-50/30 ${selectedConv ? 'flex' : 'hidden md:flex'}`}>
            {selectedConv ? (
              <>
                ...
                <div className="bg-white border border-slate-200/80 rounded-2xl p-3.5 mb-4 flex items-center justify-between shadow-sm">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className="font-extrabold text-slate-900 text-sm">{otherParticipant.fullName || 'Livreur'}</div>
                      <div className="text-xs text-slate-500 flex items-center gap-2">
                        {otherParticipant.phone && (
                          <a href={`tel:${otherParticipant.phone}`} className="hover:text-blue-600 flex items-center gap-1 font-semibold">
                ...
                >
                  {messages.length === 0 ? (
                    <div className="text-center py-20 text-slate-400 text-xs">
                      <MessageSquare className="w-10 h-10 text-blue-500/30 mx-auto mb-2" />
                        ...
                        >
                          <div
                            className={`p-3.5 px-4 rounded-3xl max-w-[82%] text-xs shadow-sm space-y-1.5 ${
                              isMe
                                ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-medium rounded-tr-sm shadow-blue-500/10'
                  ...
                  <div className="relative">
                    ...
                    <form onSubmit={handleSendMessage} className="flex items-center gap-2 pt-1">
                      <button
                        ...
                        className="w-11 h-11 rounded-full bg-blue-600 hover:bg-blue-700 text-white flex items-center justify-center shadow-md shrink-0 transition-transform active:scale-95 cursor-pointer"
                        title="Ajouter une pièce jointe"
                      >
                        {showAttachMenu ? <X className="w-5 h-5" /> : <Plus className="w-5 h-5" />}
                      </button>

                      <div className="flex-1 bg-white border border-slate-200 focus-within:border-blue-500 rounded-full px-4 py-2.5 flex items-center gap-2 shadow-sm transition-all">
                        <input
                          type="text"
                          value={newMessage}
                      ...
                      <button
                        ...
                        className="w-11 h-11 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white flex items-center justify-center shadow-md shrink-0 disabled:opacity-40 transition-all active:scale-95 cursor-pointer"
                      >
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-slate-400 text-xs py-16 bg-white border border-slate-200/80 rounded-2xl">
                ...
                <span className="font-extrabold text-slate-900 text-base">Messagerie Instantanée</span>
