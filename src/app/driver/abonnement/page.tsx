...
import { ArrowLeft, CreditCard, ShieldCheck, CheckCircle2, Clock, AlertTriangle, RefreshCw, Calendar, Sparkles, ArrowRight, Bike } from 'lucide-react';
...
export default function DriverAbonnementPage() {
  ...
  const fetchSubscriptionData = async () => {
    ...
    try {
      ...
      console.error('Erreur chargement abonnement livreur:', err);
    } finally {
      setLoadingSub(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Erreur soumission abonnement livreur');

      setSuccessMsg('Paiement d\'abonnement livreur soumis avec succès ! Il est en cours de vérification par l\'administration.');
      setTransactionReference('');
      fetchSubscriptionData();
    } catch (err: any) {
      setError(err.message || 'Erreur lors de la soumission de l\'abonnement livreur');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-amber-500 selection:text-white flex flex-col justify-between">
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {/* EN-TÊTE PAGE LIVREUR */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm">
          <div className="flex items-center space-x-4">
            <Link href="/driver" className="p-2.5 rounded-2xl bg-slate-100 border border-slate-200 text-slate-700 hover:text-slate-950 transition-colors">
              <ArrowLeft className="w-5 h-5" />
            </Link>
            <div>
              <h1 className="text-2xl font-extrabold text-slate-950">Suivi de Mon Abonnement Livreur</h1>
              <p className="text-xs text-slate-600 font-medium mt-0.5">Accès illimité aux courses de Ouagadougou avec 0% de commission sur vos revenus</p>
            </div>
          </div>
          ...
          >
            <RefreshCw className={`w-4 h-4 text-orange-600 ${loadingSub ? 'animate-spin' : ''}`} />
        ...
        {loadingSub ? (
          <div className="bg-white p-8 rounded-3xl border border-slate-200 text-center py-12 space-y-3 shadow-sm">
            ...
            <p className="text-sm font-semibold text-slate-600">Chargement des données d'abonnement livreur...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-sm space-y-6">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Statut de votre compte livreur</span>
                {isSubActive ? (
              ...
              {isSubActive ? (
                <div className="space-y-4">
                  <div className="space-y-1">
                    <h2 className="text-3xl font-black text-slate-950">Abonnement Mensuel Livreur</h2>
                ...
                <div className="space-y-4 p-5 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900">
                  <div className="flex items-center gap-2 font-extrabold text-sm text-amber-800">
                    ...
                    <span>Paiement de 1 000 FCFA soumis le {new Date(pendingPayment.createdAt).toLocaleDateString('fr-FR')}</span>
                  </div>
                  <p className="text-xs text-amber-950 leading-relaxed">
                    Votre paiement de référence <strong>"{pendingPayment.transactionReference}"</strong> via <strong>{pendingPayment.paymentMethod.toUpperCase()}</strong> est en cours d'examen par l'administrateur. Dès confirmation, votre statut livreur sera automatiquement renouvelé pour 30 jours.
                  </p>
                </div>
              ) : (
                <div className="space-y-3 p-5 rounded-2xl bg-rose-50 border border-rose-200 text-rose-900">
                  <div className="flex items-center gap-2 font-extrabold text-sm text-rose-800">
                    ...
                    <span>Aucun abonnement livreur actif</span>
                  </div>
                  <p className="text-xs text-rose-950 leading-relaxed">
                    Abonnez-vous pour 1 000 FCFA / mois afin de continuer à recevoir les propositions de livraison à Ouagadougou et conserver 100% de vos gains sans commission.
                  </p>
                </div>
              )}
              ...
              <div className="pt-4 border-t border-slate-100 space-y-3">
                <div className="text-xs font-bold uppercase tracking-wider text-slate-400">Ce que comprend l'abonnement livreur :</div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-slate-700 font-medium">
                  <div className="flex items-center space-x-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>0% de commission sur toutes vos courses</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>Réception directe des alertes GPS</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>Propositions de prix directement aux clients</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>Badge Livreur Vérifié à Ouagadougou</span>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm space-y-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="font-extrabold text-slate-950 text-base">Abonnement Livreur</h3>
                  <div className="text-xs text-orange-600 font-bold">1 000 FCFA / 30 jours</div>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-xs space-y-1.5 text-slate-800">
                  ...
                  {paymentMethod === 'orange_money' && (
                    <div>
                      ...
                      <div className="mt-1"><code className="bg-slate-200 text-orange-700 px-1.5 py-0.5 rounded font-mono font-bold">*144*2*1*06887330*1000#</code></div>
                    </div>
                  )}
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-3 bg-slate-50 border border-slate-300 rounded-xl text-slate-900 text-xs outline-none focus:bg-white focus:ring-2 focus:ring-orange-500 transition-all placeholder-slate-400"
                  />
                </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-sm space-y-6">
          <div className="flex items-center justify-between border-b border-slate-100 pb-4">
            <div>
              <h2 className="text-xl font-extrabold text-slate-950">Historique des Paiements d'Abonnement Livreur</h2>
