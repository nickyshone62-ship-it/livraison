    ...
    <div className="min-h-screen bg-slate-950 text-white pb-12">
      <header className="border-b border-slate-800 bg-slate-900/60 backdrop-blur-xl sticky top-0 z-40">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center space-x-4">
          ...
          <h1 className="text-xl font-bold text-white">Messagerie Client</h1>
        </div>
      </header>
              ...
              conversations.map((c) => (
                <button
                  ...
                  className={`w-full p-3 rounded-2xl text-left border transition-all ${
                    selectedConv?.id === c.id ? 'bg-orange-500/10 border-orange-500 text-white' : 'bg-slate-950 border-slate-800 text-slate-400 hover:border-slate-700'
                  }`}
                >
                  <div className="font-bold text-sm text-white">{c.client?.fullName || 'Client'}</div>
            ...
            {selectedConv ? (
              <>
                <div className="border-b border-slate-800 pb-3 mb-4">
                  <div className="font-bold text-white text-base">{selectedConv.client?.fullName || 'Client'}</div>
                  <div className="text-xs text-slate-400">Tél: {selectedConv.client?.phone || 'N/A'}</div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-3 mb-4 max-h-[350px] pr-2">
                  {messages.map((m) => (
                    <div key={m.id} className={`p-3 rounded-2xl max-w-[80%] text-xs ${
                      m.senderId === selectedConv.driverId ? 'ml-auto bg-orange-500 text-white font-medium' : 'bg-slate-800 text-slate-200'
                    }`}>
                      {m.content}
                    </div>
                  ))}
                ...
                <form onSubmit={handleSendMessage} className="flex items-center space-x-2 pt-2 border-t border-slate-800">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-slate-950 border border-slate-800 rounded-2xl text-white text-xs outline-none focus:ring-2 focus:ring-orange-500"
                  />
                  <button
                    type="submit"
                    disabled={sendLoading}
                    className="p-3 rounded-2xl bg-orange-500 hover:bg-orange-600 text-white font-bold"
                  >
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-slate-500 text-xs">
                ...
                <span>Sélectionnez une conversation pour échanger avec un client.</span>
              </div>
            )}
          </div>
        </div>
